...
Org Babel has the =var= header argument to pass values to source code blocks.
For example, in =C= you can write =:var i=1= and then use =i= inside the source
code block. Internally, this parameter also /declares/ the variable =i= as an
=int=. This mechanism is also implemented for other programming languages
supported in Babel.
...
preferable to let programmers choose the appropriate type of a variable assigned
to 1: =Integer=, =Natural= or =Positive=. So, we decided that it's better to not
to implement this mechanism in Ada/SPARK.

Instead, we can take advantage of the =:var= header argument to perform /literal
substitutions/. That is, to replace the indicated variables with the value
assigned to it. For example, in the case that a source code block wants to open
a file in the same directory than the orgmode file, the compiled code is
executed from a different directory, so a path is required. The path depends on
the current working directory of the orgmode file, which can be different for
virtually each user.
...
To used =:var= argument in the same way as it is implemented in other
programming languages, first provide the type:
